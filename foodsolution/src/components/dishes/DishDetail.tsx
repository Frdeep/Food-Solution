...
import { ChevronLeft, Edit3, Trash2, Lightbulb, TrendingUp, AlertTriangle, Check } from 'lucide-react';
...
export function DishDetail({ dishId, onBack, onEdit }: DishDetailProps) {
  ...
  const emoji = CATEGORY_EMOJIS[dish.category] || '🍴';
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-white to-surface-secondary">
      {/* Header */}
      <header className="sticky top-0 z-10 flex items-center justify-between p-3 sm:p-4 bg-white/90 backdrop-blur-lg safe-area-inset-top">
        <button 
          onClick={onBack}
        ...
        <div className="flex gap-1">
          ...
          <button 
            ...
            className="p-2 rounded-full hover:bg-danger/10 active:bg-danger/20 transition-colors touch-manipulation"
          >
      ...
      <div className="flex-1 overflow-y-auto pb-24 overscroll-contain">
        ...
        <motion.div 
          ...
          className="relative h-40 sm:h-48 bg-gradient-to-br from-amber-100 to-orange-100 flex items-center justify-center mx-4 rounded-3xl overflow-hidden"
        >
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/50 to-transparent">
            ...
            <h1 className="text-xl sm:text-2xl font-bold text-white">{dish.name}</h1>
          </div>
        </motion.div>
        ...
        <div className="p-4 space-y-4">
          ...
          >
            <GlassCard className="p-4 sm:p-6">
              <h2 className="font-semibold text-slate-900 mb-4 text-sm sm:text-base">RENTABILITÉ</h2>
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-4">
                <div className="text-center p-2 sm:p-3 bg-slate-50 rounded-xl">
                  ...
                  <p className="text-lg sm:text-xl font-bold text-slate-900">{formatPrice(dish.sellingPrice)}</p>
                </div>
                <div className="text-center p-2 sm:p-3 bg-slate-50 rounded-xl">
                  <p className="text-xs text-slate-500">Coût MP</p>
                  <p className="text-lg sm:text-xl font-bold text-slate-900">{formatPrice(dish.costPrice)}</p>
                </div>
              </div>
              ...
              <div className="flex justify-center">
                ...
                >
                  {(status === 'excellent' || status === 'good') && <Check className="w-3 h-3 sm:w-4 sm:h-4 mr-1" />}
                  {(status === 'warning' || status === 'danger') && <AlertTriangle className="w-3 h-3 sm:w-4 sm:h-4 mr-1" />}
                  <span className="text-xs sm:text-sm">
                    {status === 'excellent' && 'Excellent'}
                    {status === 'good' && 'Bon'}
                    {status === 'warning' && 'À améliorer'}
                    {status === 'danger' && 'Non rentable'}
                  </span>
                </Badge>
              </div>
            </GlassCard>
          ...
          >
            <GlassCard className="p-4">
              <h2 className="font-semibold text-slate-900 mb-3 text-sm sm:text-base">COMPOSITION</h2>
              ...
              <div className="space-y-2">
                {dish.ingredients.map((ing, index) => {
                  ...
                  return (
                    <div key={index} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
                      <span className="text-slate-700 text-sm truncate flex-1">{ing.productName || product?.name}</span>
                      <div className="flex items-center gap-3 sm:gap-4 flex-shrink-0">
                        <span className="text-xs sm:text-sm text-slate-500">
                          {ing.quantity} {ing.unit}
                        </span>
                        <span className="font-medium text-slate-900 text-sm w-14 text-right">
                ...
                <div className="flex items-center justify-between pt-2 font-bold border-t border-slate-200">
              ...
              <GlassCard className="p-4 border-2 border-brand-primary/20">
                <div className="flex items-start gap-3">
                  <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-brand-primary/10 flex items-center justify-center flex-shrink-0">
                    <Lightbulb className="w-4 h-4 sm:w-5 sm:h-5 text-brand-primary" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-slate-900 mb-2 text-sm">Recommandation Foodyx</h3>
                    <ul className="space-y-1.5">
                      {analysis.recommendations.map((rec, index) => (
                        <li key={index} className="text-xs sm:text-sm text-slate-600">
                          • {rec}
                        </li>
                      ))}
                    </ul>
